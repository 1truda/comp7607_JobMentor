...
def main():
    ...
    exp_parts = initial_code.split("```")
    explanation = exp_parts[2].strip()
    data = {
        "explanation": explanation
    }
    with open("explanation_output.json", "w", encoding="utf-8") as json_file:
    ...
    for round_number in tqdm(range(ROUNDS)):
        ...
        if flask8_errors:
            error_suggestions = "\n".join(flask8_errors)
            error_suggestions += "\n\nPlease generate the modified code and code interpreter only"
